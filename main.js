...
/*№8
1. Создать счетчик для подсчета кол-ва каждого товара
2.

первые 3 раза alert выведет тело функции
function a() {
    alert(this);
}
т.к. это последнее переопределенное значение переменной "a", при этом данная
функция не вызывается
------

в последнем вызове будет строковое представление объекта window, т.к. this
в функции "a" будет ссылаться на него (т.к. в качестве контекста передан null)
